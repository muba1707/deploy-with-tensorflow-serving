...
class CaptureHand():
    ...
    def capture_video(self):
        ...
        text_image = np.zeros((400, 400, 1), dtype = "uint8")
        ...
        while camera.isOpened():
            ...
            if self.is_bg_captured == 1:  # this part wont run until background captured
                ...
                url = 'http://localhost:8038/v1/models/mymodel:predict'
                ...
                # # get the predictions
                # http = HTTPSession()
                # json_response = http.request('post', url, data)
                predictions = json.loads(json_response.text)
                class_pred = classes[np.argmax(predictions["predictions"][0])]
